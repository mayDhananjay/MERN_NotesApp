...
import { Lock as LockIcon, User as UserIcon, Mail as MailIcon, ArrowRight, AlertCircle, StickyNote } from 'lucide-react';
...
const Register = ({ setUser }) => {
  ...
  // Password validation helper matching App.jsx
  ...
  const handleSubmmit = async (e) => {
    ...
    // Username length validation matching server.js
    ...
    // Password validations matching App.jsx
  ...
  return (
    <div className="relative min-h-screen bg-stone-50/70 text-stone-900 flex flex-col items-center justify-center px-4 selection:bg-amber-200 selection:text-stone-900 overflow-hidden" id="register-page-container">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="flex flex-col items-center mb-8 text-center" id="register-logo-header">
        ...
        {/* Elegant Form Card */}
        <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-[0_12px_32px_rgba(0,0,0,0.06)] p-8 relative" id="register-card">
          ...
          <div className="text-center mb-8 pt-2">
            <h2 className="text-xl font-bold text-stone-900 tracking-tight">Register</h2>
            <p className="text-xs text-stone-500 mt-1 font-medium">
              Create an account to start syncing notes securely!
            </p>
          </div>

          {error && (
            <div className="flex items-start gap-2.5 bg-rose-50 border border-rose-100 text-rose-800 p-3.5 rounded-xl text-xs font-semibold leading-relaxed mb-6" id="register-error-alert">
              <AlertCircle className="w-4 h-4 text-rose-500 shrink-0 mt-0.5" />
              <p className="flex-1">{error}</p>
            </div>
          )}

          <form onSubmit={handleSubmmit} className="space-y-5" id="register-form">
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-widest text-stone-500 mb-2 cursor-pointer" htmlFor="register-username">
                Username
              </label>
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-widest text-stone-500 mb-2 cursor-pointer" htmlFor="register-email">
                Email Address
              </label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-stone-400">
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-widest text-stone-500 mb-2 cursor-pointer" htmlFor="register-password">
                Password
              </label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-stone-400">
            ...
            <div className="bg-stone-50 p-3 rounded-xl border border-stone-200 text-[11px] space-y-1.5 text-stone-600">
              ...
              <div className="flex items-center gap-1.5">
                <span className={password.length >= 6 ? "text-emerald-600 font-bold" : "text-stone-400"}>
                  {password.length >= 6 ? "✓" : "○"} At least 6 characters ({password.length}/6)
                </span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className={/[!@#$%^&*(),.?":{}|<>]/g.test(password) ? "text-emerald-600 font-bold" : "text-stone-400"}>
                  {/[!@#$%^&*(),.?":{}|<>]/g.test(password) ? "✓" : "○"} Contains special character (e.g., @, $, !, %, *, ?, &, #)
                </span>
              </div>
            </div>
          ...
          <div className="mt-8 pt-6 border-t border-stone-100 text-center">
            <p className="text-xs text-stone-500 font-semibold">
              ...
              <Link className="text-amber-600 hover:text-amber-700 hover:underline font-bold transition-colors" to="/login">
                Login
              </Link>
            </p>
          </div>
